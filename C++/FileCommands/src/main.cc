...
int Create(char *argv[]) {
    ...
    ::off_t sizeForFTruc = std::atoi(argv[4]);
    std::size_t sizeForMap = std::atoi(argv[4]);

    // Opens the file and resizes the file
    int fd = proj3::open(filePath,
                         proj3::O_RDWR | proj3::O_CREAT | proj3::O_TRUNC,
                         0666);
    if (fd == -1) {
        return 1;
    }
    int ftrunc = proj3::ftruncate(fd, sizeForFTruc);

    // Check if ftruncate failed
    if (ftrunc == -1) {
        std::cout << "Error: ftruncate failed. Exiting..." << std::endl;
        proj3::close(fd);
        return 1;
    }
    ...
    void *map = proj3::mmap(nullptr, sizeForMap,
    ...
    if (proj3::msync(map, sizeForMap, proj3::MS_SYNC) == -1) {
        proj3::munmap(map, sizeForMap);
        proj3::close(fd);
        return 1;
    }

    if (proj3::munmap(map, sizeForMap) == -1) {
        proj3::close(fd);
        return 1;
    }
...
int Insert(char *argv[]) {
    ...
    if (proj3::ftruncate(fd, newSizeForFile) == -1) {
        proj3::close(fd);
        return 1;
    }

    // Maps the file to a processed address space
    void *map = proj3::mmap(nullptr, newSizeForFile,
        proj3::PROT_READ | proj3::PROT_WRITE,
        proj3::MAP_SHARED, fd, 0);
    ...
    for (std::size_t i = offset; i < (offset + bytesIncoming); ++i) {
        ...
        if (readByte == EOF) {
            ...
            proj3::munmap(map, newSizeForFile);
            proj3::ftruncate(fd, f_len);
            proj3::close(fd);
            return 1;
        }
    ...
    // Flush the changes and unmap the memory space
    // and close the file
    if (proj3::msync(map, newSizeForFile, proj3::MS_SYNC) == -1) {
        proj3::munmap(map, newSizeForFile);
        proj3::ftruncate(fd, f_len);
        proj3::close(fd);
        return 1;
    ...
    if (proj3::munmap(map, newSizeForFile) == -1) {
        proj3::ftruncate(fd, f_len);
        proj3::close(fd);
        return 1;
    }
...
int Append(char *argv[]) {
    ...
    if (currentFileSize == 0 && remainingBytes > 0) {
        ...
        if (proj3::ftruncate(fd, 1) == -1) {
            proj3::close(fd);
            return 1;
        }

        // Create map
        void *oneByteMap = proj3::mmap(nullptr, 1,
            proj3::PROT_READ | proj3::PROT_WRITE,
            proj3::MAP_SHARED, fd, 0);

        // Check if map returned MAP_FAILED
        if (oneByteMap == reinterpret_cast<void *>(-1)) {
            proj3::ftruncate(fd, originalFileSize);
            proj3::close(fd);
            return 1;
        }
        ...
        if (readOneByte == EOF) {
            proj3::munmap(oneByteMap, 1);
            if (proj3::ftruncate(fd, originalFileSize) == -1) {
                proj3::close(fd);
                return 1;
            }
            proj3::close(fd);
        ...
        if (proj3::msync(oneByteMap, 1, proj3::MS_SYNC) == -1) {
            proj3::munmap(oneByteMap, 1);
            proj3::ftruncate(fd, originalFileSize);
            proj3::close(fd);
            return 1;
        ...
        if (proj3::munmap(oneByteMap, 1) == -1) {
            proj3::ftruncate(fd, originalFileSize);
            proj3::close(fd);
            return 1;
        }
    ...
    while (remainingBytes > 0) {
        ...
        if (proj3::ftruncate(fd, newFileSize) == -1) {
            proj3::ftruncate(fd, originalFileSize);
            proj3::close(fd);
            return 1;
        }
        ...
        // Create map
        void *map = proj3::mmap(nullptr, newFileSize,
            proj3::PROT_READ | proj3::PROT_WRITE,
            proj3::MAP_SHARED, fd, 0);
        ...
        // Update variables
        if (proj3::msync(map, newFileSize, proj3::MS_SYNC) == -1) {
            proj3::munmap(map, newFileSize);
            proj3::ftruncate(fd, originalFileSize);
            proj3::close(fd);
            return 1;
        ...
        if (proj3::munmap(map, newFileSize) == -1) {
            proj3::ftruncate(fd, originalFileSize);
            proj3::close(fd);
            return 1;
        }
